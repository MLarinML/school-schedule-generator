...
console.log('🧪 Тестирование конфигурации email...\n');
...
const checks = [
  ...
  },
  {
    name: 'SMTP_USER',
    pattern: /SMTP_USER="([^"]+)"/,
    expected: 'your-email@yandex.ru',
    description: 'SMTP пользователь'
  },
  {
    name: 'SMTP_PASS',
    pattern: /SMTP_PASS="([^"]+)"/,
    expected: 'your-yandex-app-password',
...
if (allPassed) {
  ...
  exec('curl -X POST http://localhost:3001/api/email/test-connection', (error, stdout, stderr) => {
    if (error) {
      ...
      try {
        ...
        if (result.success) {
          console.log('🎉 Email сервис работает отлично!');
        } else {
  ...
  console.log('\n📚 Или следуйте инструкциям в YANDEX_EMAIL_SETUP.md');
}
